...
if(empty($title) || empty($type) || empty($address) || empty($uri) || empty($description) || empty($owner_name) || empty($owner_email)) {
  ...
  $insert = mysql_query("INSERT INTO places (approved, title, type, address, uri, description, owner_name, owner_email,lat,lng,sector) VALUES (0, '$title', '$type', '$address', '$uri', '$description', '$owner_name', '$owner_email',0,0,'NA')") or die(mysql_error());
